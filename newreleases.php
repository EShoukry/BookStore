...
if ($sort_sql_values[$sort] == 'author_id') {
    $result = $mysqli->query("SELECT DISTINCT books.book_id, b_name, b_price, b_picture, b_description, b_rate FROM books, books_authors, authors WHERE books.book_id = books_authors.book_id AND authors.author_id= books_authors.author_id and books.b_is_new='1' ORDER BY authors.a_name");
} else {
    $result = $mysqli->query("SELECT book_id, b_name, b_price, b_picture, b_description, b_rate FROM books WHERE books.b_is_new='1' ORDER BY " . $sort_sql_values[$sort]);
}
